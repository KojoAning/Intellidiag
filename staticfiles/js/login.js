    ...
    $('#login-button').click(function (event) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success) {
                    // Display success message
                    $('#success-message').html('<div class="success-message">' + response.message + '</div>');
                    ...
                    if (response.errors && Object.keys(response.errors).length > 0) {
                        for (const field in response.errors) {
                            if (response.errors.hasOwnProperty(field) && field !== '__all__') {
                                const fieldErrors = response.errors[field];
                                const errorContainer = document.getElementById(`${field}-error`);
                                if (errorContainer) {
                                    errorContainer.innerHTML = fieldErrors.map(error => `<div>${error}</div>`).join('');
